...
const fetchSocialLinks = async (): Promise<SocialMediaLink[]> => {
  const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000'}/api/social-media-links`, {
    params: {
...
const fetchResumeDownload = async (): Promise<ResumeDownload | null> => {
  const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000'}/api/resume-downloads`, {
    params: {
      sort_by: 'created_at',
      sort_order: 'desc',
      limit: 1
...
const GV_Footer: React.FC = () => {
  ...
  const copyrightText = `© ${currentYear} ${developerName}. All rights reserved.`;
        ...
        <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
          <div className="md:flex md:items-center md:justify-between">
            ...
            <div className="mt-8 md:mt-0 md:order-1 flex flex-col items-center md:items-end">
              ...
              <p className="mt-4 text-center text-sm text-gray-500 md:mt-2">
                {copyrightText}
              </p>
              
              <div className="mt-2 flex space-x-6">
                <Link to="/privacy" className="text-sm text-gray-500 hover:text-gray-700 transition-colors duration-200">
                  Privacy Policy
                </Link>
